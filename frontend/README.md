...
Все временные значения помечены комментарием `TEMPORARY`. Отсутствующие изображения показываются как нейтральные placeholders, поэтому для наполнения достаточно указать `imageUrl` или `avatarUrl`.
