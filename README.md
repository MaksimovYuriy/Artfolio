...
Artfolio — публичное авторское портфолио с закрытой административной панелью. Владелец может редактировать профиль и социальные ссылки, загружать работы, менять порядок и управлять публикацией.
...
маршрут, статус, размер ответа и длительность. Заголовок `X-Request-ID` возвращается клиенту и может
использоваться для поиска конкретного запроса в логах.
...
Ошибки REST API возвращаются в едином JSON-формате со стабильным машинным кодом и request ID:
